			...
			altEditor.prototype,
			{
				...
				_openEditModal: function () {
					...
					for (var j in columnDefs) {
						...
						// the if statement is additional for the proper rendering of the content. 
						if (columnDefs[j].type === "text") {
							$(selector).find(jquerySelector).val(selectedValue); // new code 
						} else {
						...
						//#region Custom code for disabling the rowkey on Edit pop up
						if (columnDefs[j].name === "rowKey") {
							$(selector).find(jquerySelector).attr('readonly', true);
						}
						//#endregion
					}
				...
				*/
				...
				_deleteRowCallback: function (response, status, more) {
					...
					$(selector).modal("hide"); // custom code not part of the lib
					...
					this.s.dt.ajax.reload(null, false); // new code
				...
				_addRowCallback: function (response, status, more) {
					...
					//this.s.dt.row.add(data).draw(false); // original code
					this.s.dt.ajax.reload(null, false); // new code
				...
				_editRowCallback: function (response, status, more) {
					...
					//this.s.dt.draw();
					// custom code
					this.s.dt.ajax.reload(null, false);
